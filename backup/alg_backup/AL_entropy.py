...
class ALEntropy:
    ...
    def entropy_train(self):
        data, target, entropy = zip(*self.buffer)
        data, target, entropy = torch.cat(data), torch.cat(target), np.concatenate(entropy)
        highest_entropy = entropy.argsort()[-self.budgets:]
        anchor_loader = InfiniteDataLoader(TensorDataset(data[highest_entropy].cpu(), target[highest_entropy].cpu()), weights=None,
                                           batch_size=self.config.train.train_bs, num_workers=self.config.num_workers)
        ...
        for i, (data, target) in enumerate(anchor_loader):
            ...
            loss.backward()
            optimizer.step()
            if i > self.config.atta.ActiveTTA.steps:
                break
    ...
    def adapt_on_env(self, env_id):
        ...
        with torch.no_grad():
            for data, target in self.buffer_fast_loader[env_id]:
                data, target = data.to(self.config.device), target.to(self.config.device)
                outputs = self.model(data)
                entropy = self.softmax_entropy(outputs)
                self.buffer.append((data, target, entropy.cpu().numpy()))
        # budget = 100
        # buf_size = 10
        # preheat = 100
