...
class ALCluster:
    ...
    def cluster_train(self):
        ...
        anchor_loader = InfiniteDataLoader(TensorDataset(data[closest].cpu(), target[closest].cpu()), weights=None,
                                           batch_size=self.config.train.train_bs, num_workers=self.config.num_workers)
    ...
    def adapt_on_env(self, env_id):
        ...
        with torch.no_grad():
            for data, target in self.buffer_fast_loader[env_id]:
                ...
                self.buffer.append((data, target, feats.cpu().numpy()))
        # budget = 100
        # buf_size = 10
        # preheat = 100
