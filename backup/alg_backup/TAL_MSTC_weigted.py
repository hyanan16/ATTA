...
class TALMSTCweighted:
    def __init__(self, config: Conf):
        super(TALMSTCweighted, self).__init__()
    ...
    def adapt_on_env(self, env_id):
        beta_func = torch.distributions.beta.Beta(0.5, 0.5)
        with torch.no_grad():
            for data, target in tqdm(self.buffer_fast_loader[env_id]):
                ...
                if self.anchors:
                    ...
                    entropy = self.softmax_entropy(self.fc(anchors_feats.to(self.config.device))).cpu()

                    entropy = entropy - entropy.min() + 1e-6
                    entropy = entropy / (entropy.max() + 1e-6)
                    sample_weight = sample_weight * beta_func.log_prob(entropy).exp()
                    ...
                    entropy = self.softmax_entropy(self.fc(feats.to(self.config.device))).cpu()

                    entropy = entropy - entropy.min() + 1e-6
                    entropy = entropy / (entropy.max() + 1e-6)
                    sample_weight = sample_weight * beta_func.log_prob(entropy).exp()
                    kmeans = KMeans(n_clusters=self.n_clusters, n_init=10).fit(feats, sample_weight=sample_weight)
                ...
                if self.cold_start > 0:
                    self.cold_start -= 1
                    self.n_clusters += self.cold_start
                else:
                    self.n_clusters += 2
                    self.cluster_train()
        # self.cluster_train()
